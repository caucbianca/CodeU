	...
	public static ArrayList<Move> rearrangeCars(int[] input, int[] output) {
		...
		while (!wrongCarsPositions.isEmpty()) {
			...
			if (movingCar == NO_CAR) {
				...
				int wrongPlacedCar = wrongCarsPositions.keySet().iterator().next();
				int currentPlace = wrongCarsPositions.get(wrongPlacedCar);

				/* Move the car in the free parking space and store the move*/
				wrongCarsPositions.replace(wrongPlacedCar, currentPlace, freeSpace);
				moves.add(new Move(wrongPlacedCar, currentPlace, freeSpace));
